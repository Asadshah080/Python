...
print("\n")
print("processes\t   Arrival time\t         Burst time\t          periority\t       waiting time\t     turnaround time\t")
for i in range(0,len(bursttime)):
  print(str(processes[i])+"\t\t\t"+str(arrivaltime[i])+"\t\t\t"+str(bursttime[i])+"\t\t\t"+str(periority[i])+"\t\t\t"+str(waitingtime[i])+"\t\t\t"+str(turnaroundtime[i]))
  print("\n")
print("Gantt Chart")
